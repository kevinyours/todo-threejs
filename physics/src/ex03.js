...
export default function example() {
  ...
  cannonWorld.gravity.set(0, -10, 0); // 중력: -10

  // Contact Material: 두 재질이 부딪쳤을 때에 대한 물리 속성 설정
  const defaultMaterial = new CANNON.Material("default");
  const rubberMaterial = new CANNON.Material("rubber");
  const ionMaterial = new CANNON.Material("iron");
  ...
  const ironMaterial = new CANNON.ContactMaterial(
    ionMaterial,
    defaultMaterial,
    {
      friction: 0.5,
  ...
  cannonWorld.addContactMaterial(ironMaterial);

  const floorShape = new CANNON.Plane();
  const floorBody = new CANNON.Body({
    mass: 0, // 무게, 바닥 역할을 하므로 중력의 영향을 받으면 안됨
  ...
  const sphereShape = new CANNON.Box(new CANNON.Vec3(0.25, 2.5, 0.25));
  ...
  function draw() {
    ...
    let cannonStepTiem = 1 / 60; // 60hz 초당 60번
    if (delta < 0.01) cannonStepTiem = 1 / 120; // 120hz 초당 120번
    cannonWorld.step(cannonStepTiem, delta, 3);
